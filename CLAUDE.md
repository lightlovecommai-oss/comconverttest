...
- `CTX_OPTS` — 場景畫像：不進計分，問「你最想在哪種場合說話更有影響力」3 選項
  （職場與家人／一對一銷售／一對多演講＝首頁三張場景卡＋教學三大應用場景，前後呼應）。
  每個場景帶 `need`（場景名）＋ `sym`（4 塊肌肉在該場景的具體症狀句），
  結果頁判讀＝「你選的場景 × 最弱那塊肌肉」查 `sym` 表。
  🔄 2026-09-07 拍板：原「錨定單一對象」版作廢——12 題題幹各帶場景、1–5 錨點是跨情境頻率，
  根本不可能對同一次談話作答，判讀建立在假前提上（三方打架）。
  試算表欄位不動：targetContext／targetNeed 送場景名、targetDistance／targetRank 送空字串、
  **targetKeyMuscle 改存最弱大肌肉**（舊 `key` 欄位已刪）。
